// RUN: %clang_cc1 -triple x86_64-apple-darwin10 -fioc-unsigned %s -emit-llvm -o - | FileCheck %s
...
void testlongadd() {
  ...
  // CHECK:      {{^}}[[T6]]:
  // CHECK-NEXT: call void @__ioc_report_add_overflow
  li = lj + lk;
}
...
void testlongsub() {
  ...
  // CHECK-NEXT: br i1 [[T5]], label %[[T6:.+]], label %[[T7:.+]]
  // CHECK:      {{^}}[[T6]]:
  // CHECK-NEXT: call void @__ioc_report_sub_overflow
  li = lj - lk;
}
...
void testlongmul() {
  ...
  // CHECK-NEXT: br i1 [[T5]], label %[[T6:.+]], label %[[T7:.+]]
  // CHECK:      {{^}}[[T6]]:
  // CHECK-NEXT: call void @__ioc_report_mul_overflow
  li = lj * lk;
}
